...
class MargeParent:
    ...
    # # 各个子单的数据清洗
    # def weight_out(self,name,money,hold):
    #     """
    #
    #     Args:
    #         name: 文件名中间部分
    #         money: 目标金额
    #         hold:  1 持续交易 0 只交易到目标时间
    #
    #     Returns:
    #
    #     """
    #     try:
    #         df = pd.read_excel(f'file/child/{self.daily}_{name}_request.xlsx').drop('Unnamed: 0', axis=1)
    #     except:
    #         feishu_bot.send_msg_to_feishu(f"{self.daily} 未找到{name}的子单邮件")
    #         exit(0)
    #     try:
    #         df_old_weight = pd.read_excel(f'file/child/{name}_historical_position.xlsx').drop('Unnamed: 0', axis=1)
    #         # print(df_old_weight)
    #         new_df = pd.merge(df, df_old_weight,
    #                           on=['secCode', 'market', 'targetNumber', 'sendTime', 'endTime', 'direction', 'cal'],
    #                           how='outer')
    #         new_df = new_df.fillna(0)
    #         new_df['new_weight'] = new_df.apply(lambda x: x['weight'] - x['old_weight'], axis=1)
    #         new_df = new_df[new_df['new_weight'] != 0]
    #         new_df['new_direction'] = new_df.apply(lambda x: 2 if x['new_weight'] < 0 else 1, axis=1)
    #         new_df['new_weight'] = abs(new_df['new_weight'])
    #         new_df.drop(['old_weight', 'direction', 'weight'], axis=1, inplace=True)
    #         new_df.rename(columns={'new_weight': 'weight', 'new_direction': 'direction'}, inplace=True)
    #         df = new_df
    #     except:
    #         pass
    #     df['secCode'] = df['secCode'].apply(lambda x: "%06d" % int(x))
    #     df['Market'] = df.apply(lambda x: 1 if x['market'].upper() == 'SH' else 2, axis=1)
    #     df['new_side'] = df.apply(lambda x: 1 if x['direction'] == 1 else 2, axis=1)
    ...
    #     df['logo'] = '500_wxk'
    #     df['secCode_Market'] = df.apply(lambda x: x['secCode'] + '.' + x['market'].upper(), axis=1)
    #     df['orderQty'] = df.apply(
    #         lambda x: str(int(x['weight'] * money / float(self.colse_price[x['secCode_Market']].values[0]))), axis=1)
    #     df['True_orderQty'] = df.apply(
    #         lambda x: int(str(x['orderQty'])[:-2] + "00") if x['secCode'][0:3] != '688' else int(x['orderQty']), axis=1)
    #
    #     df['True_orderQty'] = df.apply(
    #         lambda x: 0 if x['secCode'][0:3] == '688' and x['True_orderQty'] < 200 else x['True_orderQty'], axis=1)
    #     # print(df[df['True_orderQty']<200])
    #     df.drop(['orderQty'], axis=1, inplace=True)
    #     df.rename(columns={'secCode': 'symbol', 'new_side': 'side', 'True_orderQty': 'orderQty'}, inplace=True)
    #     df['cal_action'] = f'涨幅限制=;跌幅限制=;涨跌停是否继续交易=0;过期后是否继续交易={hold};'
    #     df = df[df['orderQty'] > 100]
    #     return df[['symbol', 'orderQty', 'side', 'Market', 'sendTime', 'endTime', 'cal', 'cal_action', 'logo']]
    #
    # def target_out(self,name,hold):
    #     df = pd.read_csv(f'file/child/{self.daily}_{name}_request.csv')
    #     try:
    ...
    #     #
    #     # df_old = df.copy()
    #     # df_old.rename(columns={'targetNumber': 'old_targetNumber'}, inplace=True)
    #     # df_old.to_excel(f'file/child/500_zyy_request_old.xlsx')
    #     # print(df)
    #     df['secCode'] = df['secCode'].apply(lambda x: "%06d" % int(x))
    #     df['Market'] = df.apply(lambda x: 1 if x['market'].upper() == 'SH' else 2, axis=1)
    #     df['sendTime'] = df['sendTime'].apply(lambda x: "%04d:00" % int(x)).apply(
    ...
    #     df['endTime'] = df['endTime'].apply(lambda x: "%04d:00" % int(x)).apply(lambda x: str(x)[:2] + ':' + str(x)[2:])
    #     df['logo'] = '500_zyy'
    #     df.rename(columns={'secCode': 'symbol', 'targetNumber': 'orderQty', 'new_side': 'side'}, inplace=True)
